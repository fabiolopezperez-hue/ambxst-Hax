...
Singleton {
    ...
    // Se usa cuando no se especifica ubicación en la búsqueda de clima.
    readonly property string defaultLocation: configLocation || "Cabanillas del Campo"
    property bool _initialized: false

    // Debounce timer: agrupa cambios rápidos de config (location + unit llegan juntos)
    Timer {
        id: configDebouncer
        interval: 500
    ...
    function updateWeather() {
        ...
        root.isLoading = true;
        root.hasFailed = false;
        root.wasCancelled = false;  // <-- Resetear para la nueva petición
